  ...
  public class Pop3MimeClient:Pop3MailClient {
      ...
      string parentBoundaryEnd) 
    {
      ...
      do
      {
          ...
          //*****************************************************************************************************************
          // Added the below to ensure that the message body is always set when working with MIME MULTIPART content
          // NOTE:
          // Outlook 2007 sends two parts when sending an email as HTML (I think this is the same for other email clients):
          // 1. The first part seems to be PLAIN TEXT
          // 2. The second part seems to be HTML TEXT
          // Since this is a MULTIPART we want to set this body to the second HTML PART
          // If other clients only send one part for HTML then this method will not be called
          // If you want to set the Body to the TEXT only PART then you need to change the MimeEntityReturnCode.parentBoundaryEndFound flag to MimeEntityReturnCode.parentBoundaryStartFound
          //*****************************************************************************************************************
          if (ReturnCode == MimeEntityReturnCode.parentBoundaryEndFound && (message.Body == null || message.Body == String.Empty) && ChildPart.Body != null && ChildPart.Body != String.Empty)
          {
              message.Body = ChildPart.Body.Trim();
          }
