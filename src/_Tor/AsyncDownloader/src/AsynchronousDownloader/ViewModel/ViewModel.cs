...
{
    public class DownloadFileViewModel : BasePropChanged
    {
        ...
        private void OpenFile(object obj)
        {
            ...
            Process.Start("explorer.exe", $"/select,\"{file.DownloadLocation}\"");
        }
        ...
        {
            var file = obj as DownloadFile;
            return (file?.DownloadStatus == DownloadStatus.NotDownloaded);
        }
        ...
        private void Download(object obj)
        {
            ...
            webClient.DownloadFileAsync(new Uri(file.Uri), file.DownloadLocation = $"{Directory.GetCurrentDirectory()}\\{file.Filename}");
        }
        ...
        void webClient_DownloadProgressChanged(object sender, DownloadProgressChangedEventArgs e)
        {
            ...
            downloadFile.DownloadPercentageString =
                $"{int.Parse(Math.Truncate(((double.Parse(e.BytesReceived.ToString()) / double.Parse(e.TotalBytesToReceive.ToString())) * 100)).ToString())} %";
            ...
            downloadFile.DownloadTime =
                $"{int.Parse(Math.Truncate(customWebClient.Clock.Elapsed.TotalSeconds).ToString())} s";
            downloadFile.DownloadSpeed =
                $"{(e.BytesReceived / 1024d / customWebClient.Clock.Elapsed.TotalSeconds).ToString("0.00")} kb/s";
        }
